...
public partial class Form1 : Form
{
    ...
    {
        InitializeComponent();
        this.Text = "Guillotine Ray v1.0.8";
        SetupUi();
        
        // 起動時の初期化�E Load イベントで行う�E�ハンドル未作�Eエラー回避のため�E�E        this.Load += (s, e) => {
            SetLanguage("JP");
    ...
    private void SetupUi()
    {
        ...
        // フォルダ選抁E        _labels["folder"] = new Label { Text = "チE��プレートフォルダ", Bounds = new Rectangle(left, 12, 180, 18), ForeColor = Color.LightGray };
        ...
        // しきぁE��
        _labels["thresh"] = new Label { Text = "しきぁE�� (0.8 - 0.95)", Bounds = new Rectangle(left, 65, 180, 18), ForeColor = Color.LightGray };
        ...
        // 間隔
        _labels["interval"] = new Label { Text = "監視間隁E(ミリ私E", Bounds = new Rectangle(left, 118, 180, 18), ForeColor = Color.LightGray };
        numInterval = new NumericUpDown { Bounds = new Rectangle(left, 138, 100, 25), Maximum = 60000, Value = 5000, Minimum = 10 };

        // ROI
        _labels["roi"] = new Label { Text = "監視篁E�� (X, Y, 幁E 高さ)", Bounds = new Rectangle(left, 172, 240, 18), ForeColor = Color.LightGray };
        for (int i = 0; i < 4; i++) {
            numRoi[i] = new NumericUpDown { Bounds = new Rectangle(left + i * 60, 192, 55, 25), Maximum = 9999 };
        }
        numRoi[2].Value = 300; numRoi[3].Value = 300;

        btnRoi = new Button { Text = "画面から篁E��を選抁E, Bounds = new Rectangle(left, 225, 240, 32), FlatStyle = FlatStyle.Flat, BackColor = Color.FromArgb(60, 60, 60) };
        btnRoi.Click += (s, e) => {
    ...
    private void SetLanguage(string lang)
    {
        ...
        _labels["folder"].Text = isJp ? "チE��プレートフォルダ" : "Template Folder";
        _labels["thresh"].Text = isJp ? "しきぁE�� (0.8 - 0.95)" : "Threshold (0.8 - 0.95)";
        _labels["interval"].Text = isJp ? "監視間隁E(ミリ私E" : "Interval (ms)";
        _labels["roi"].Text = isJp ? "監視篁E�� (X, Y, 幁E 高さ)" : "ROI (X, Y, W, H)";
        btnRoi.Text = isJp ? "画面から篁E��を選抁E : "Select ROI on Screen";
        btnStart.Text = isJp ? "監視開姁E(START)" : "START";
    ...
    private async void Start()
    {
        ...
        try { 
            ...
            Log(_currentLang == "JP" ? "チE��プレートを読み込みました、E : "Templates loaded.");
        } catch (Exception ex) { Log(ex.Message); return; }

        _active = true; _cts = new CancellationTokenSource();
        UpdateUiState(true);
        Log(_currentLang == "JP" ? "監視を開始しました、E : "Monitoring started.");
    ...
    private void Stop()
    {
        ...
        UpdateUiState(false);
        Log(_currentLang == "JP" ? "監視を停止しました、E : "Monitoring stopped.");
    }
    ...
    private async Task Loop(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            if (res.Found) {
                Log(_currentLang == "JP" ? $"検�E: {res.Name} (一致度: {res.Score:F3})" : $"Hit: {res.Name} ({res.Score:F3})");
                _matcher.SaveDebugImage(bmp, res, roi);
                await InputController.ClickAtAsync(res.Center.X, res.Center.Y);
                await Task.Delay(50, ct); 
            } else if (res.Score == -100) {
                Log(_currentLang == "JP" ? $"警呁E {res.Name} が監視篁E��より大きいです、E : $"Warning: {res.Name} is larger than ROI.");
